...
// src/utils.ts
import { ethers, zeroPadValue, parseUnits, formatUnits, TypedDataEncoder, ZeroAddress } from "ethers";
...
async function getPermitDomain(token, provider) {
  ...
  if (token.realOriginChainId && token.realOriginContractAddress && token.wChainId !== token.realOriginChainId) {
    ...
    const salt2 = ethers.keccak256(packed);
...
async function getSwapEvm(params) {
  ...
  const res = await fetch2(`${addresses_default.PRICE_URL}/get-swap/evm?${query}`, {
    method: "GET",
    redirect: "follow"
  });
  await check5xxError(res);
...
async function submitSwiftSolanaSwap(signedTx, chainName) {
  const res = await fetch2(`${addresses_default.EXPLORER_URL}/submit/v2/svm`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
async function getEstimateGasEvm(params) {
  const res = await fetch2(`${addresses_default.GAS_ESTIMATE_URL}/evm`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
async function getSvmDurableNonce(chainName, swapperAddress) {
  const res = await fetch2(`${addresses_default.SWIFT_RELAYER_URL}/nonces/assign`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
import { PublicKey as PublicKey12, SystemProgram as SystemProgram11 } from "@solana/web3.js";
...
function getCCTPV2BridgePDAs(mint, destinationChain, ledger, trader) {
  const cctpV2CoreProgramId = new PublicKey2(addresses_default.CCTPV2_CORE_PROGRAM_ID);
  const cctpV2TokenProgramId = new PublicKey2(addresses_default.CCTPV2_TOKEN_PROGRAM_ID);
  const [messageTransmitter] = PublicKey2.findProgramAddressSync(
    [Buffer2.from("message_transmitter")],
    cctpV2CoreProgramId
  );
  ...
  const [cctpDenyListAccount] = PublicKey2.findProgramAddressSync(
    [
      Buffer2.from("denylist_account"),
      ledger.toBytes()
    ],
    cctpV2TokenProgramId
  );
  const [realDenyListAccount] = PublicKey2.findProgramAddressSync(
    [
      Buffer2.from("denylist_account"),
      trader.toBytes()
    ],
...
function getEvmMctpCreateOrderParams(quote, destinationAddress, referrerAddress, signerChainId) {
  ...
  const tokenOut = quote.toToken.contract === ZeroAddress2 ? nativeAddressToHexString(SystemProgram2.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(
...
async function getMctpFromEvmTxPayload(quote, destinationAddress, referrerAddress, signerChainId, permit, payload) {
  ...
  if (quote.fromToken.contract === quote.mctpInputContract) {
    ...
    if (quote.hasAuction) {
      ...
      if (quote.fromToken.contract === ZeroAddress2) {
...
function decodeJupiterV6InsArgs(args) {
  ...
  const discHex = Buffer4.from(args.subarray(0, 8)).toString("hex").toLowerCase();
  const data = Buffer4.from(args.subarray(8));
...
async function unwrapSol(owner, amount, signTransaction, connection) {
  ...
  const delegate = Keypair2.generate();
  const {
    blockhash,
    lastValidBlockHeight
  } = await solanaConnection.getLatestBlockhash();
...
var payerAccountsLocation = [
  {
    ...
    programId: SystemProgram3.programId.toString(),
...
function overrideInstructionsPayer(instructions, relayer) {
  ...
  return instructions.map((instruction) => {
    const override = payerAccountsLocation.find(
      (item) => item.programId === instruction.programId && item.instructionDataHex === Buffer5.from(instruction.data, "base64").subarray(0, item.discriminatorLength).toString("hex") && instruction.accounts.length > item.accountIndex
    );
    if (override) {
...
function getAnchorInstructionData(name) {
  ...
  return Buffer5.from(sha256(preimage)).subarray(0, 8);
}
...
function validateJupSwapInstruction(instruction, validDestAccount, sameSourceAndDestWallet = false) {
  const JUP_PROGRAM_ID = new PublicKey3("JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4");
...
function createPayloadWriterCreateInstruction(payer, payloadAccount, payload, nonce) {
  ...
  return new TransactionInstruction({
    keys,
    programId: new PublicKey3(addresses_default.PAYLOAD_WRITER_PROGRAM_ID),
    data: insData
  });
}
...
function sandwichInstructionInCpiProxy(instruction, skipProxy = false) {
  ...
  return new TransactionInstruction({
    ...
    programId: new PublicKey3(addresses_default.CPI_PROXY_PROGRAM_ID),
...
import { blob as blob5, struct as struct5, u16 as u164, u8 as u85 } from "@solana/buffer-layout";
...
function getWormholePDAs(supplierProgram, chainName) {
  ...
  if (chainName === "solana") {
    wormholeProgramId = new PublicKey4(addresses_default.WORMHOLE_PROGRAM_ID);
    wormholeShimProgramId = new PublicKey4(addresses_default.WORMHOLE_SHIM_POST_MESSAGE_PROGRAM_ID);
  } else if (chainName === "fogo") {
    wormholeProgramId = new PublicKey4(addresses_default.WORMHOLE_PROGRAM_ID_FOGO);
    wormholeShimProgramId = new PublicKey4(addresses_default.WORMHOLE_SHIM_POST_MESSAGE_PROGRAM_ID);
  } else {
  ...
  const [shimMessage] = PublicKey4.findProgramAddressSync(
    [emitter.toBuffer()],
    wormholeShimProgramId
  );
  const [shimEventAuth] = PublicKey4.findProgramAddressSync(
    [Buffer6.from("__event_authority")],
    wormholeShimProgramId
  );
  return {
    bridgeConfig,
...
function createMctpBridgeWithFeeInstruction(ledger, toChain, mintAddress, relayerAddress, feeSolana, fromChain) {
  ...
  const wormholeShimProgramId = new PublicKey5(addresses_default.WORMHOLE_SHIM_POST_MESSAGE_PROGRAM_ID);
  ...
  const cctpMessage = Keypair3.generate();
  const accounts = [
...
function createMctpBridgeLedgerInstruction(params) {
  ...
  const relayer = new PublicKey5(params.relayerAddress);
...
async function createMctpFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  if (options.customPayload) {
    customPayloadAccount = PublicKey5.findProgramAddressSync(
      [
        Buffer7.from("PAYLOAD"),
        relayer.toBuffer(),
        (() => {
          const buf = Buffer7.alloc(2);
          buf.writeUInt16LE(customPayloadNonce, 0);
          return buf;
        })()
      ],
      new PublicKey5(addresses_default.PAYLOAD_WRITER_PROGRAM_ID)
    )[0];
    instructions.push(
      sandwichInstructionInCpiProxy(createPayloadWriterCreateInstruction(
        relayer,
        customPayloadAccount,
        Buffer7.from(options.customPayload),
        customPayloadNonce
      ))
    );
  }
  if (quote.fromToken.contract === quote.mctpInputContract) {
    ...
    instructions.push(
      sandwichInstructionInCpiProxy(createAssociatedTokenAccountInstruction(relayer, ledgerAccount, ledger, new PublicKey5(quote.mctpInputContract)))
    );
    instructions.push(
...
import { ethers as ethers2, ZeroAddress as ZeroAddress3 } from "ethers";
function createSwiftOrderHash(quote, swapperAddress, destinationAddress, referrerAddress, randomKeyHex, customPayload) {
  ...
  const _tokenIn = fromTokenContract === ZeroAddress3 ? nativeAddressToHexString(SystemProgram5.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(fromTokenContract, sourceChainId);
  ...
  const refAddress = referrerAddress ? Buffer8.from(hexToUint8Array(nativeAddressToHexString(referrerAddress, referrerChainId))) : SystemProgram5.programId.toBuffer();
  ...
  if (quote.swiftVersion === "V2") {
    ...
    if (customPayload) {
      customPayloadHash = Buffer8.from(hexToUint8Array(ethers2.keccak256(Buffer8.from(customPayload))));
    } else {
      customPayloadHash = SystemProgram5.programId.toBuffer();
    }
...
function createSwiftInitInstruction(params) {
  const { quote } = params;
  const mint = quote.swiftInputContract === ZeroAddress3 ? solMint : new PublicKey6(quote.swiftInputContract);
  const destinationChainId = getWormholeChainIdByName(quote.toChain);
  ...
  const accounts = quote.swiftVersion === "V2" ? [
    ...
    {
      pubkey: params.customPayloadAccount || new PublicKey6(addresses_default.SWIFT_V2_PROGRAM_ID),
      isWritable: false,
      isSigner: false
    },
    { pubkey: mint, isWritable: false, isSigner: false },
  ...
  const refAddress = params.referrerAddress ? Buffer8.from(hexToUint8Array(nativeAddressToHexString(params.referrerAddress, referrerChainId))) : SystemProgram5.programId.toBuffer();
  let minMiddleAmount;
  ...
  InitSwiftLayout.encode({
    ...
    nativeInput: quote.swiftInputContract === ZeroAddress3 ? 1 : 0,
    ...
    destinationChain: destinationChainId,
    tokenOut: Buffer8.from(hexToUint8Array(_tokenOut)),
  ...
  return new TransactionInstruction3({
    keys: accounts,
    data,
    programId: new PublicKey6(quote.swiftVersion === "V2" ? addresses_default.SWIFT_V2_PROGRAM_ID : addresses_default.SWIFT_PROGRAM_ID)
  });
}
async function createSwiftFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}, customPayload, customPayloadAccount) {
  ...
  const randomKey = new PublicKey6(createSwiftRandomKey(quote));
  if (!Number(quote.deadline64)) {
  ...
  const chainDestBuffer = Buffer8.alloc(2);
  chainDestBuffer.writeUInt16LE(getWormholeChainIdByName(quote.toChain));
  const [state] = quote.swiftVersion === "V2" ? PublicKey6.findProgramAddressSync(
    [Buffer8.from("STATE_SOURCE"), hash, chainDestBuffer],
    new PublicKey6(addresses_default.SWIFT_V2_PROGRAM_ID)
  ) : PublicKey6.findProgramAddressSync(
    [Buffer8.from("STATE_SOURCE"), hash],
    new PublicKey6(addresses_default.SWIFT_PROGRAM_ID)
  );
  let tokenProgramId;
  if (quote.swiftVersion === "V2") {
    tokenProgramId = quote.swiftInputContractStandard === "spl2022" ? new PublicKey6(addresses_default.TOKEN_2022_PROGRAM_ID) : new PublicKey6(addresses_default.TOKEN_PROGRAM_ID);
  } else {
    tokenProgramId = new PublicKey6(addresses_default.TOKEN_PROGRAM_ID);
  }
  console.log({ tokenProgramId });
  const swiftInputMint = quote.swiftInputContract === ZeroAddress3 ? solMint : new PublicKey6(quote.swiftInputContract);
  ...
  if (quote.fromToken.contract === quote.swiftInputContract) {
    ...
    if (quote.swiftInputContract === ZeroAddress3) {
      instructions.push(
        sandwichInstructionInCpiProxy(SystemProgram5.transfer({
          fromPubkey: trader,
  ...
  instructions.push(sandwichInstructionInCpiProxy(createSwiftInitInstruction({
    ...
    tokenProgramId,
    customPayload,
    customPayloadAccount: customPayloadAccount ? new PublicKey6(customPayloadAccount) : void 0
  }), options.skipProxyMayanInstructions));
  const totalLookupTables = await getAddressLookupTableAccounts(_lookupTablesAddress.concat(_swapAddressLookupTables), connection);
  lookupTables = totalLookupTables.slice(0, _lookupTablesAddress.length);
  if (swapInstructions.length > 0) {
...
import { blob as blob4, struct as struct4, u16 as u163, u8 as u84, u32 } from "@solana/buffer-layout";
import { Buffer as Buffer9 } from "buffer";
var FastMctpBridgeLayout = struct4([
  blob4(8, "instruction")
]);
function createFastMctpBridgeInstruction(ledger, trader, toChain, mintAddress, relayerAddress, feeSolana) {
  const TOKEN_PROGRAM_ID = new PublicKey7(addresses_default.TOKEN_PROGRAM_ID);
  const cctpCoreProgramId = new PublicKey7(addresses_default.CCTPV2_CORE_PROGRAM_ID);
  const cctpTokenProgramId = new PublicKey7(addresses_default.CCTPV2_TOKEN_PROGRAM_ID);
  const fastMctpProgram = new PublicKey7(addresses_default.FAST_MCTP_PROGRAM_ID);
  const relayer = new PublicKey7(relayerAddress);
  const mint = new PublicKey7(mintAddress);
  const ledgerAccount = getAssociatedTokenAddress(
    mint,
    ledger,
    true
  ...
  if (feeSolana && feeSolana > BigInt(0)) {
    ...
    relayerAccount = new PublicKey7(addresses_default.FAST_MCTP_PROGRAM_ID);
  }
  ...
  const cctpMessage = Keypair5.generate();
  const accounts = [
    { pubkey: ledger, isWritable: true, isSigner: false },
    { pubkey: ledgerAccount, isWritable: true, isSigner: false },
    { pubkey: relayer, isWritable: true, isSigner: true },
  ...
  const data = Buffer9.alloc(FastMctpBridgeLayout.span);
  ...
  const bridgeIns = new TransactionInstruction4({
    keys: accounts,
    data,
...
var FastMctpInitOrderLayout = struct4([
  blob4(8, "instruction")
]);
function createFastMctpInitOrderInstruction(ledger, trader, toChain, mintAddress, relayerAddress, feeSolana) {
  ...
  const data = Buffer9.alloc(FastMctpInitOrderLayout.span);
  ...
  const initOrderIns = new TransactionInstruction4({
    keys: accounts,
    data,
    programId: fastMctpProgram
  });
...
var FastMctpBridgeLedgerLayout = struct4([
  blob4(8, "instruction"),
  blob4(32, "destAddress"),
  blob4(8, "amountInMin"),
  blob4(8, "gasDrop"),
  blob4(8, "feeRedeem"),
  blob4(8, "feeSolana"),
  ...
  u84("feeRateRef"),
  u163("keyRnd"),
  blob4(8, "maxCircleFee"),
  u32("minFinalityThreshold"),
  u84("mode")
]);
function createFastMctpBridgeLedgerInstruction(params) {
  const user = new PublicKey7(params.swapperAddress);
  const relayer = new PublicKey7(params.relayerAddress);
  const mint = new PublicKey7(params.mintAddress);
  const ledgerAccount = getAssociatedTokenAddress(mint, params.ledger, true);
  const destinationChainId = getWormholeChainIdByName(params.toChain);
  ...
  const refAddress = params.referrerAddress ? Buffer9.from(hexToUint8Array(
    nativeAddressToHexString(params.referrerAddress, destinationChainId)
  )) : SystemProgram6.programId.toBuffer();
  ...
  const accounts = [
    ...
    { pubkey: relayer, isWritable: true, isSigner: true },
    { pubkey: ledgerAccount, isWritable: false, isSigner: false },
    { pubkey: params.customPayload || new PublicKey7(addresses_default.FAST_MCTP_PROGRAM_ID), isWritable: false, isSigner: false },
    { pubkey: mint, isWritable: false, isSigner: false },
    { pubkey: SystemProgram6.programId, isWritable: false, isSigner: false }
  ];
  const data = Buffer9.alloc(FastMctpBridgeLedgerLayout.span);
  ...
  return new TransactionInstruction4({
    keys: accounts,
    data,
    programId: new PublicKey7(addresses_default.FAST_MCTP_PROGRAM_ID)
  });
}
var FastMctpOrderLedgerLayout = struct4([
  blob4(8, "instruction"),
  blob4(32, "destAddress"),
  blob4(8, "amountInMin"),
  blob4(8, "gasDrop"),
...
function createFastMctpOrderLedgerInstruction(params) {
  ...
  const feeSolana = getSafeU64Blob(params.feeSolana);
  const maxCircleFee = getSafeU64Blob(params.maxCircleFee);
  const tokenOut = Buffer9.from(hexToUint8Array(
    nativeAddressToHexString(params.tokenOut, destinationChainId)
  ));
  const refAddress = params.referrerAddress ? Buffer9.from(hexToUint8Array(
    nativeAddressToHexString(params.referrerAddress, destinationChainId)
  ...
  const data = Buffer9.alloc(FastMctpOrderLedgerLayout.span);
...
async function createFastMctpFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  let swapInstructions = [];
  let createSwapTpmTokenAccountInstructions = [];
  const tmpSwapTokenAccount = Keypair5.generate();
  let swapMessageV0Params = null;
  _lookupTablesAddress.push(getLookupTableAddress(quote.fromChain));
  const fastMctpProgram = new PublicKey7(addresses_default.FAST_MCTP_PROGRAM_ID);
  const user = new PublicKey7(swapperAddress);
  const relayer = new PublicKey7(relayerAddress);
  ...
  const ledgerSeedPrefix = quote.hasAuction ? "LEDGER_ORDER" : "LEDGER_BRIDGE";
  const [ledger] = PublicKey7.findProgramAddressSync(
    [
      Buffer9.from(ledgerSeedPrefix),
      user.toBytes(),
      (() => {
  ...
  const ledgerAccount = getAssociatedTokenAddress(
    new PublicKey7(quote.fastMctpInputContract),
    ledger,
    true
  );
  if (quote.toChain === "sui" && !quote.toToken.verifiedAddress) {
  ...
  const customPayloadNonce = Math.floor(Math.random() * 65e3);
  if (options.customPayload) {
    customPayloadAccount = PublicKey7.findProgramAddressSync(
      [
      ...
      new PublicKey7(addresses_default.PAYLOAD_WRITER_PROGRAM_ID)
    )[0];
    instructions.push(
      sandwichInstructionInCpiProxy(createPayloadWriterCreateInstruction(
        relayer,
        customPayloadAccount,
        Buffer9.from(options.customPayload),
        customPayloadNonce
      ))
    );
  }
  ...
  if (quote.fromToken.contract === quote.fastMctpInputContract) {
    ...
    instructions.push(
      sandwichInstructionInCpiProxy(createAssociatedTokenAccountInstruction(relayer, ledgerAccount, ledger, new PublicKey7(quote.fastMctpInputContract)))
    );
    instructions.push(
      sandwichInstructionInCpiProxy(createSplTransferInstruction(
        getAssociatedTokenAddress(
...
async function createHyperCoreDepositFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  let swapInstructions = [];
  let createSwapTpmTokenAccountInstructions = [];
  const tmpSwapTokenAccount = Keypair6.generate();
  let swapMessageV0Params = null;
  ...
  const [payloadAccount] = PublicKey8.findProgramAddressSync(
    [
      Buffer10.from("PAYLOAD"),
      trader.toBuffer(),
      (() => {
        const buf = Buffer10.alloc(2);
  ...
  const mctpRandomKey = Keypair6.generate();
  const mctpProgram = new PublicKey8(addresses_default.MCTP_PROGRAM_ID);
  const fastMctpRandomKey = Math.floor(Math.random() * 65e3);
  const fastMctpProgram = new PublicKey8(addresses_default.FAST_MCTP_PROGRAM_ID);
  let ledger = void 0;
  if (quote.type === "MCTP") {
    const [_ledger] = PublicKey8.findProgramAddressSync(
      [
        Buffer10.from("LEDGER_BRIDGE"),
        trader.toBytes(),
  ...
  if (quote.fromToken.contract === quote.hyperCoreParams.initiateTokenContract) {
    if (quote.suggestedPriorityFee > 0) {
      instructions.push(
        ComputeBudgetProgram5.setComputeUnitPrice({
          microLamports: quote.suggestedPriorityFee
        })
      );
    }
...
async function createMonoChainFromSolanaInstructions(quote, swapperAddress, destinationAddress, referrerAddress, connection, options = {}) {
  ...
  const toMint = quote.toToken.contract === ZeroAddress4 ? solMint : new PublicKey9(quote.toToken.contract);
...
// src/solana/solanaSwap.ts
var STATE_SIZE = 420;
var SwapLayout = struct5([
  u85("instruction"),
  u85("stateNonce"),
  blob5(8, "amount"),
  blob5(8, "minAmountOut"),
  blob5(8, "deadline"),
  blob5(8, "feeSwap"),
  blob5(8, "feeReturn"),
  blob5(8, "feeCancel"),
  blob5(8, "gasDrop"),
  u164("destinationChain"),
  blob5(32, "destinationAddress"),
  u85("unwrapRedeem"),
  u85("unwrapRefund"),
  u85("mayanFeeNonce"),
  u85("referrerFeeNonce")
]);
async function createSwapFromSolanaInstructions(quote, swapperWalletAddress, destinationAddress, referrerAddresses, connection, options = {}) {
  ...
  let instructions = [];
  const solanaConnection = connection ?? new Connection7("https://rpc.ankr.com/solana");
  ...
  const [mayanFee, mayanFeeNonce] = PublicKey10.findProgramAddressSync(
    [Buffer11.from("MAYANFEE")],
    mayanProgram
  );
  const [referrerFee, referrerFeeNonce] = PublicKey10.findProgramAddressSync(
    [
      Buffer11.from("REFERRERFEE"),
      referrerAddr.toBuffer()
    ],
    mayanProgram
  );
  const msg1 = Keypair8.generate();
  const msg2 = Keypair8.generate();
  const [state, stateNonce] = PublicKey10.findProgramAddressSync(
    [
      Buffer11.from("V2STATE"),
      Buffer11.from(msg1.publicKey.toBytes()),
      Buffer11.from(msg2.publicKey.toBytes())
    ],
    mayanProgram
  );
  ...
  const delegate = Keypair8.generate();
  ...
  const swapKeys = [
    ...
    { pubkey: SYSVAR_CLOCK_PUBKEY3, isWritable: false, isSigner: false },
  ...
  const destAddress = Buffer11.from(
    hexToUint8Array(
  ...
  const deadline = BigInt(quote.deadline64);
  const swapData = Buffer11.alloc(SwapLayout.span);
  const swapFields = {
    instruction: 101,
    stateNonce,
    amount: getSafeU64Blob(amount),
  ...
  const swapInstruction = new TransactionInstruction7({
    keys: swapKeys,
    data: swapData,
...
async function swapFromSolana(quote, swapperWalletAddress, destinationAddress, referrerAddresses, signTransaction, connection, extraRpcs, sendOptions, jitoOptions, instructionOptions) {
  ...
  if (quote.gasless) {
    feePayer = new PublicKey10(quote.relayer);
    const {
      publicKey: noncePubkey,
      nonce
    } = await getSvmDurableNonce(quote.fromChain, swapperWalletAddress);
    instructions = [SystemProgram7.nonceAdvance({
      noncePubkey: new PublicKey10(noncePubkey),
      authorizedPubkey: feePayer
    }), ..._instructions];
    recentBlockhash = nonce;
    lastValidBlockHeight = 0;
  } else {
    feePayer = new PublicKey10(swapperWalletAddress);
    instructions = _instructions;
  ...
  if (quote.gasless) {
    const serializedTrx = Buffer11.from(signedTrx.serialize()).toString("base64");
...
function getEvmFastMctpBridgeParams(quote, destinationAddress, referrerAddress, signerChainId, customPayload) {
  ...
  return {
    ...
    customPayload: customPayload ? `0x${Buffer13.from(customPayload).toString("hex")}` : "0x",
...
function getEvmFastMctpBridgeTxPayload(quote, destinationAddress, referrerAddress, signerChainId, payload) {
  ...
  const fastMctpContract = new Contract3(contractAddress, MayanFastMctpArtifact_default.abi);
  let data;
  let value;
    ...
    if (quote.hasAuction) {
      ...
      if (quote.fromToken.contract === ZeroAddress7) {
        ...
        return {
          data,
          to: addresses_default.MAYAN_FORWARDER_CONTRACT,
          value: toBeHex3(fastMctpPayloadIx._params.amountIn),
          chainId: signerChainId,
          _forwarder: {
            method: forwarderMethod,
            params: forwarderParams
...
function getUsdcDepositInitiatorMctpTxPayload(quote, swapperAddress, destinationAddress, usdcPermitSignature) {
  ...
  const initiatorContract = new Contract4(
    quote.hyperCoreParams.initiateContractAddress,
    HCDepositInitiatorArtifact_default.abi
  );
  const signatureBuf = Buffer14.from(hexToUint8Array(usdcPermitSignature));
  ...
  if (quote.fromToken.contract.toLowerCase() === quote.hyperCoreParams.initiateTokenContract.toLowerCase()) {
    ...
    if (quote.fromToken.contract === ZeroAddress8) {
      ...
      return {
        data,
        to: addresses_default.MAYAN_FORWARDER_CONTRACT,
        value: toBeHex4(BigInt(quote.effectiveAmountIn64)),
        chainId: signerChainId,
        _forwarder: {
          method: forwarderMethod,
          params: forwarderParams
...
import { Contract as Contract5, toBeHex as toBeHex5, ZeroAddress as ZeroAddress9 } from "ethers";
...
function getEvmMonoChainTxPayload(quote, destinationAddress, referrerAddress) {
  ...
  const monoChainContract = new Contract5(
  ...
  if (quote.fromToken.contract === ZeroAddress9) {
    value = toBeHex5(quote.effectiveAmountIn64);
  } else {
    value = toBeHex5(0);
  }
  return {
...
function getMonoChainFromEvmTxPayload(quote, destinationAddress, referrerAddress, signerChainId, permit) {
  ...
  if (quote.fromToken.contract === ZeroAddress9) {
    ...
    return {
      data,
      to: addresses_default.MAYAN_FORWARDER_CONTRACT,
      value: toBeHex5(monoChainPayloadIx._params.amountIn),
      chainId: signerChainId,
      _forwarder: {
        method: forwarderMethod,
        params: forwarderParams
...
function getEvmSwapParams(quote, destinationAddress, referrerAddress, signerAddress, signerChainId, payload) {
  const mayanProgram = new PublicKey12(addresses_default.MAYAN_PROGRAM_ID);
  const [mayanMainAccount] = PublicKey12.findProgramAddressSync(
    [Buffer15.from("MAIN")],
    mayanProgram
  );
  const recipient = getAssociatedTokenAddress(
    new PublicKey12(quote.fromToken.mint),
    mayanMainAccount,
    true
  );
  const amountIn = BigInt(quote.effectiveAmountIn64);
  ...
  const criteria = {
    ...
    unwrap: unwrapRedeem,
    customPayload: payload ? `0x${Buffer15.from(payload).toString("hex")}` : "0x"
  };
...
async function getSwapFromEvmTxPayload(quote, swapperAddress, destinationAddress, referrerAddresses, signerAddress, signerChainId, payload, permit, options) {
  ...
  const forwarderContract = new Contract6(
    addresses_default.MAYAN_FORWARDER_CONTRACT,
    MayanForwarderArtifact_default.abi
  );
  const mayanSwap = new Contract6(contractAddress, MayanSwapArtifact_default.abi);
  let forwarderMethod;
  let forwarderParams;
  let value;
  const _permit = permit || ZeroPermit;
...
import { SystemProgram as SystemProgram14, Keypair as SolanaKeypair } from "@solana/web3.js";
import { ethers as ethers5, ZeroAddress as ZeroAddress12 } from "ethers";
async function createSwiftFromSuiMoveCalls(quote, swapperAddress, destinationAddress, referrerAddress, payload, suiClient, options) {
  ...
  let inputCoin;
  if (quote.fromToken.contract === quote.swiftInputContract) {
    tx = options?.builtTransaction ?? new Transaction5();
    inputCoin = await resolveInputCoin(
      BigInt(quote.effectiveAmountIn64),
      swapperAddress,
      quote.swiftInputContract,
    ...
    tx = Transaction5.from(serializedTx);
    inputCoin = outCoin;
  }
...
async function addInitSwiftOrderMoveCalls(quote, swapperAddress, destinationAddress, referrerAddress, swiftPackageId, feeManagerPackageId, payload, suiClient, options) {
  const destChainId = getWormholeChainIdByName(quote.toChain);
  const tx = options?.builtTransaction ?? new Transaction5();
  ...
  const tokenOut = quote.toToken.contract === ZeroAddress12 ? nativeAddressToHexString(SystemProgram14.programId.toString(), getWormholeChainIdByName("solana")) : nativeAddressToHexString(quote.toToken.contract, quote.toToken.wChainId);
  const amountOutMin = getAmountOfFractionalAmount(
    quote.minAmountOut,
    Math.min(8, quote.toToken.decimals)
  );
  ...
  if (referrerAddress) {
    ...
    referrerHex = nativeAddressToHexString(
      SystemProgram14.programId.toString(),
      getWormholeChainIdByName("solana")
    );
  }
  ...
  let customPayloadHash;
  if (payload) {
    customPayloadHash = Buffer16.from(hexToUint8Array(ethers5.keccak256(Buffer16.from(payload))));
  } else {
    customPayloadHash = SystemProgram14.programId.toBuffer();
  }
  const commonArguments = [
    tx.pure.u8(payloadType),
    tx.pure.address(swapperAddress),
  ...
  const randomKey = options?.randomKey ? Buffer16.from(options.randomKey) : SolanaKeypair.generate().publicKey.toBuffer();
  const [initOrderTicket] = tx.moveCall({
    package: swiftPackageId,
    module: "init_order",
    function: "prepare_initialize_order",
